...
public class Day4 {
    ...
    public static void main(String... args) {
        Day4 d = new Day4();
        d.caesarCipher("aczupnetwp-mfyyj-opalcexpye-977[peyac]\n" +
    ...
    public class SortSpecs implements Comparable<SortSpecs> {
        private int occurrences;
        private Character letter;

        public SortSpecs(int occurrences, Character letter) {
            this.occurrences = occurrences;
            this.letter = letter;
        }

        @Override
        public int compareTo(SortSpecs a) {
            if (this.occurrences == a.occurrences) {
                return this.letter.compareTo(a.letter);
            }
            if (this.occurrences > a.occurrences) {
                return -1;
            } else {
    ...
    public int getSumOfSectorIDs(String input) {
        String[] rooms = input.split("\n");
        int total = 0;
        for (int i = 0; i < rooms.length; i++) {
            String[] pieces = rooms[i].split("-");
            int code = Integer.parseInt(pieces[pieces.length-1].substring(0,3));
            HashMap<Character, Integer> counter = new HashMap<>();
            for (int j = 0; j < pieces.length-1; j++) {
                for (int k = 0; k < pieces[j].length(); k++) {
                    if (counter.containsKey(pieces[j].charAt(k))) {
                        counter.put(pieces[j].charAt(k), counter.get(pieces[j].charAt(k)) + 1);
                    } else {
                        counter.put(pieces[j].charAt(k), 1);
                    }
                }
            }
            Set<Map.Entry<Character, Integer>> set = counter.entrySet();
            ArrayList<SortSpecs> specs = new ArrayList<>();
            for (Map.Entry<Character, Integer> entry : set) {
                SortSpecs spec = new SortSpecs(entry.getValue(), entry.getKey());
                specs.add(spec);
            }
            Collections.sort(specs);
            String correctCode = "";
            int count = 0;
            for (SortSpecs s : specs) {
                System.out.println(s.letter + ":" + s.occurrences);
                if (count < 5)
                    correctCode += s.letter;
                count++;
            }
            String givenCode = pieces[pieces.length-1].substring(4,9);
            if (givenCode.equals(correctCode)) {
                total += code;
            }
            System.out.println("correct: " + correctCode);
            System.out.println("actual: " + givenCode);
        }
        return total;
    }

    public void caesarCipher(String input) {
        String[] rooms = input.split("\n");
        for (int i = 0; i < rooms.length; i++) {
            String[] pieces = rooms[i].split("-");
            int code = Integer.parseInt(pieces[pieces.length-1].substring(0,3));
            String sectorID = pieces[pieces.length-1].substring(4,9);
            String name = "";
            for (int j = 0; j < pieces.length-1; j++) {
                String word = "";
                for (int k = 0; k < pieces[j].length(); k++) {
                    if ((char)(pieces[j].charAt(k) + (code % 26)) > 'z') {
                        word += (char)(pieces[j].charAt(k) - (26 - (code % 26)));
                    } else {
                        word += (char)(pieces[j].charAt(k) + (code % 26));
                    }
                }
                name += word + " ";
            }
